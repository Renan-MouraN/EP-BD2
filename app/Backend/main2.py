...
DATABASE_URL = "postgresql://user:password@host:port/petmatch"
...
def update_adocao(adocao_id: int, adocao: AdocaoUpdate, db: Session = Depends(get_db)):
...
def delete_adocao(adocao_id: int, db: Session = Depends(get_db)):
    db_adocao = db.query(Adocao).filter(Adocao.id_adocao == adocao_id).first()
    if db_adocao is None:
        raise HTTPException(status_code=404, detail="Adocao not found")
    db.delete(db_adocao)
...
def update_item_pedido(item_id: int, item_pedido: ItemPedidoUpdate, db: Session = Depends(get_db)):
...
def update_depoimento(depoimento_id: int, depoimento: DepoimentoUpdate, db: Session = Depends(get_db)):
...
def delete_depoimento(depoimento_id: int, db: Session = Depends(get_db)):
    db_depoimento = db.query(Depoimento).filter(Depoimento.id_depoimento == depoimento_id).first()
    if db_depoimento is None:
        raise HTTPException(status_code=404, detail="Depoimento not found")
...
def update_prestador(prestador_id: int, prestador: PrestadorUpdate, db: Session = Depends(get_db)):
...
def update_prestador_servico(prestador_serv_id: int, prestador_servico: PrestadorServicoUpdate, db: Session = Depends(get_db)):
...
def delete_prestador_servico(prestador_serv_id: int, db: Session = Depends(get_db)):
    db_prestador_servico = db.query(PrestadorServico).filter(PrestadorServico.id_prestador_serv == prestador_serv_id).first()
    if db_prestador_servico is None:
        raise HTTPException(status_code=404, detail="PrestadorServico not found")
...
def update_pedido_servico(pedido_servico_id: int, pedido_servico: PedidoServicoUpdate, db: Session = Depends(get_db)):
...
def delete_pedido_servico(pedido_servico_id: int, db: Session = Depends(get_db)):
    db_pedido_servico = db.query(PedidoServico).filter(PedidoServico.id_pedido_servico == pedido_servico_id).first()
    if db_pedido_servico is None:
        raise HTTPException(status_code=404, detail="PedidoServico not found")
